...
public abstract class Type {
    ...
    /**
     * Get pointer type by optional dereference of 'this' type
     * if necessary.
     *
    ...
    public static class PointerType extends AddressType {
        ...
        public PointerType(Location loc, Type baseType) {
            super(loc, baseType); // A pointer is an address
         ...
         */
        @Override
        public PointerType resolveType() {
            if (!resolved) {
                ...
                if (baseType instanceof IdRefType) {
                    if (!((IdRefType)baseType).defined()) {
                        errors.error("pointer base type not defined",
                                baseType.loc);
                    }
                } else {
        ...
        public ExpNode coerce(ExpNode exp) throws IncompatibleTypes {
            Type fromType = exp.getType();
            /* nil is a valid element of any pointer type */
            if (fromType.equals(Predefined.NIL_TYPE)) {
                errors.debugMessage("Coerced " + fromType + " to " +
                        this);
                return exp;
            }
            throw new IncompatibleTypes(
        ...
        public boolean equals(Type other) {
            return other instanceof PointerType &&
                    getBaseType().equals(((PointerType) other).getBaseType());
        }

        @Override
        public String toString() {
            // return getName() + "(" + "^" + baseType.getName() + ")";
    ...
    public static class RecordType extends Type {
        ...
        /* Map of fields, set up after RecordType is resolved for efficient lookup*/
        protected SortedMap<String, Field> fieldMap; // null until commenced resolution

        public RecordType(Location loc,
                          List<Field> fields) {
            // Type size will need to be resolved later
            super(loc, 0);
            this.fields = fields;
            fieldMap = null;
        }
         ...
         * @return the field, or null if not a field
         */
        ...
        public int getOffset(String id) {
            assert resolved && containsField(id);
        ...
        @Override
        public Type resolveType() {
            // System.out.println("Resolving record " + this);
            if(!resolved) {
                        if(fieldMap == null) {
                            resolveFieldMap();
                        }
            ...
            resolved = true;
            // System.out.println("Resolved record " + name + " to " + this);
            return this;
        }

        /**
         * Check for duplicates, and set up field map for efficient lookup
         */
        private void resolveFieldMap() {
            fieldMap = new TreeMap<>();
            for (Field field : fields) {
                if (fieldMap.containsKey(field.id)) {
                    errors.error("duplicate field", field.loc);
                } else {
        ...
        /** Resolves field types and allocates offsets for fields of record
         * @return size of record
         */
        private int allocFields() {
            // Offsets for new fields start at end of parent fields
            int offset = 0;
        ...
        public String toString() {
            ...
            String sep = "";
            for (Field field : fields) {
                result.append(sep).append(field.toString());
                sep = "; ";
            }
         return result + " END";
        }
    ...
    public static class Field {
        ...
        public Field(Location loc, String id, Type t) {
            this.loc = loc;
            this.id = id;
            this.type = t;
        }
